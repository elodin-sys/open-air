...
def _print_summary(summary: dict) -> None:
    ...
    print(
        f"reference model for {summary['concept']}{' (dry run)' if summary.get('dry_run') else ''}"
    )
    print(
    ...
    print(
        ...
        + (
            f", dropped {cleaning.get('dropped_count')} (largest {1000 * cleaning.get('dropped_largest_extent_m', 0):.0f} mm)"
            if cleaning.get("dropped_count")
            else ""
        )
    )
    ...
    print(
        f"  datum {level.get('mode')}: pitch rotation {level.get('pitch_rotation_applied_deg', 0.0):.3f}°"
    )
    anchor = summary.get("anchor_check") or {}
    if anchor.get("anchor_span_m"):
        print(
            f"  span {overall['span_m']:.4f} m vs anchor {anchor['anchor_span_m']:.4f} m ({100 * anchor['relative_deviation']:.2f}%) ok={anchor['ok']}"
        )
    else:
        print(f"  span {overall['span_m']:.4f} m (no anchor supplied)")
    print(
        f"  length {overall['length_m']:.4f} m, height {overall['height_m']:.4f} m, resolution {1000 * align['resolution_m']:.2f} mm"
    )
...
def _compare(args: argparse.Namespace) -> int:
    ...
    if not geometry_json.is_file():
        print(
            f"error: {geometry_json} not found; run python -m openair.geometry run first",
            file=sys.stderr,
        )
        return 2
    ...
    print(
        f"  model->reference p50 {1000 * m2r['p50_m']:.1f} mm, p95 {1000 * m2r['p95_m']:.1f} mm, max {1000 * m2r['max_m']:.1f} mm"
    )
    for view, item in result["silhouettes"].items():
        print(
            f"  {view}: IoU {item['iou']:.3f}, reference-only {1e4 * item['reference_only_m2']:.1f} cm², model-only {1e4 * item['model_only_m2']:.1f} cm²"
        )
    for name, stats in result["components"].items():
        if "p95_m" in stats:
            print(
                f"  {name}: p95 {1000 * stats['p95_m']:.1f} mm, mean {1000 * stats['mean_m']:.1f} mm"
            )
    for key, check in result["checks"].items():
        basis = f" ({check['basis']})" if check.get("basis") else ""
        print(
            f"  check {key}{basis}: {check['got']:.4f} vs {check['limit']:.4f} -> {'ok' if check['ok'] else 'FAIL'}"
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    ingest = sub.add_parser(
        "ingest", help="align and measure a reference triangle mesh"
    )
    ingest.add_argument("mesh", help="STL/PLY/OBJ/3MF/GLB triangle mesh")
    ingest.add_argument(
        "--concept", required=True, help="concept name (designs/<name>)"
    )
    ingest.add_argument(
        "--units",
        choices=["mm", "cm", "m", "in"],
        help="length unit of the mesh (or from sidecar)",
    )
    ingest.add_argument(
        "--sidecar",
        help="path to a <stem>.reference.json sidecar (auto-detected by default)",
    )
    ingest.add_argument(
        "--axes",
        default=None,
        help="open-air axis <- signed source axis, e.g. 'x:-z,y:-x,z:+y' (default identity)",
    )
    ingest.add_argument(
        "--datum", choices=["root-chord", "body-axis"], default="root-chord"
    )
    ingest.add_argument(
        "--expect-span-m",
        type=float,
        default=None,
        help="anchor span for the unit cross-check",
    )
    ...
    ingest.add_argument(
        ...
        type=float,
        default=0.01,
        help="drop shells below this fraction of the largest shell's area",
    )
    ingest.add_argument(
        "--fidelity-p95-mm",
        type=float,
        default=None,
        help="acceptance band for the geometry compare (default 15 mm)",
    )
    ingest.add_argument(
        "--fidelity-iou",
        type=float,
        default=None,
        help="acceptance IoU for top/side silhouettes",
    )
    ingest.add_argument(
        "--out",
        default=None,
        help="output directory (default designs/<concept>/reference)",
    )
    ingest.add_argument(
        "--sketch-dir",
        default=None,
        help="where sketch-*.png go (default parent of --out)",
    )
    ingest.add_argument(
        "--dry-run",
        action="store_true",
        help="measure and write only the review figure",
    )
    ingest.add_argument(
        "--force", action="store_true", help="replace existing sketch-*.png"
    )
    ingest.add_argument(
        "--treatment",
        choices=["reproduction", "inspiration", "requirement"],
        default="reproduction",
        help="intended sketch treatment; only reproduction emits wing.sections",
    )
    ingest.set_defaults(func=_ingest)

    compare = sub.add_parser(
        "compare", help="score an exported OpenVSP artifact against the reference"
    )
    compare.add_argument("concept", help="concept folder, name, or design YAML")
    compare.add_argument(
        "--phase", choices=["baseline", "optimized"], default="baseline"
    )
