# ceras-csr01-optimized — Elodin integration guide
...
1. Require `schema_version == "1.0"` and the identity your
   scenario expects (`concept` = `ceras-csr01`, `phase` = `optimized`).
...
- Body frame (GLB and dynamics): X forward, +Y left, +Z up, origin at
  the CG, which sits at geometry [14.3367, 0, 0] m.
- Transform: x_b = 14.3367 - x_g; y_b = -y_g;
...
References: S = 122.41 m^2, b = 34.07 m, c = 4.18 m (MAC). Mass state: 74102
kg with 18402 kg fuel aboard (`mass_properties`). With alpha in radians and
tail_incidence held at -2.1116 deg:

```text
CL = 0.0562191 + 6.64522*alpha
Cm = 0.149665 - 1.87884*alpha   (about the CG)
CD = 0.0188804 + 0.0355964*CL^2
```

Dimensionalize with qbar = 0.5*rho*V^2 and apply the section 2 adapter.
Initialize from `trim_map.csv` (cruise row: 10668 m, 231.298 m/s TAS, alpha
4.56411 deg, throttle 0.869464); re-solve equilibrium for any other condition
...
| `ceras-csr01.glb` | render mesh; spawn at scale 1.0 with no extra transforms (origin is already the CG, axes already body); embeds a polished-aluminum PBR metal material (no texture images) — restyle in the consumer if desired |
...
Mach 0 to 0.78; attached-flow alpha -12 to +12 deg; tabulated alpha -2 to 8
deg; Re/m 6.12508e+06 (single tabulated condition). Policy
