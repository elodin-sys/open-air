# diana2-optimized — Elodin integration guide
...
1. Require `schema_version == "1.0"` and the identity your
   scenario expects (`concept` = `diana2`, `phase` = `optimized`).
...
- Body frame (GLB and dynamics): X forward, +Y left, +Z up, origin at
  the CG, which sits at geometry [0.74, 0, 0] m.
- Transform: x_b = 0.74 - x_g; y_b = -y_g;
...
References: S = 1.03 m^2, b = 5 m, c = 0.206486 m (MAC). Mass state: 10.7 kg
with 0 kg fuel aboard (`mass_properties`). With alpha in radians and
tail_incidence held at -2.46888 deg:

```text
CL = -0.0665016 + 6.14048*alpha
Cm = 0.145059 - 2.21833*alpha   (about the CG)
CD = 0.0313999 + 0.0132548*CL^2
```

Dimensionalize with qbar = 0.5*rho*V^2 and apply the section 2 adapter.
Initialize from `trim_map.csv` (cruise row: 175 m, 22.4705 m/s TAS, alpha
3.74663 deg, throttle 0.18); re-solve equilibrium for any other condition
...
| `diana2.glb` | render mesh; spawn at scale 1.0 with no extra transforms (origin is already the CG, axes already body); embeds a polished-aluminum PBR metal material (no texture images) — restyle in the consumer if desired |
...
deg; Re/m 1.5173e+06 (single tabulated condition). Policy
...
Provided control groups: `aileron`, `elevator`, `flap`, `rudder`; evaluate them with the normalized conventions in section 2.
