# gtm-t2-baseline — Elodin integration guide
...
1. Require `schema_version == "1.0"` and the identity your
   scenario expects (`concept` = `gtm-t2`, `phase` = `baseline`).
...
- Body frame (GLB and dynamics): X forward, +Y left, +Z up, origin at
  the CG, which sits at geometry [1.22303, 0, 0] m.
- Transform: x_b = 1.22303 - x_g; y_b = -y_g;
...
References: S = 0.548295 m^2, b = 2.08751 m, c = 0.278983 m (MAC). Mass state:
26.195 kg with 5.22538 kg fuel aboard (`mass_properties`). With alpha in
radians and tail_incidence held at -5.05497 deg:

```text
CL = -0.0466512 + 5.00842*alpha
Cm = 0.192478 - 1.68047*alpha   (about the CG)
CD = 0.0412878 + 0.0361247*CL^2
```

Dimensionalize with qbar = 0.5*rho*V^2 and apply the section 2 adapter.
Initialize from `trim_map.csv` (cruise row: 304.8 m, 38.6 m/s TAS, alpha
6.56311 deg, throttle 0.192673); re-solve equilibrium for any other condition
...
| `gtm-t2.glb` | render mesh; spawn at scale 1.0 with no extra transforms (origin is already the CG, axes already body); embeds a polished-aluminum PBR metal material (no texture images) — restyle in the consumer if desired |
...
Mach 0 to 0.7; attached-flow alpha -12 to +12 deg; tabulated alpha -2 to 8
deg; Re/m 2.57988e+06 (single tabulated condition). Policy
