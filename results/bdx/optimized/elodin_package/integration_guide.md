# bdx-optimized — Elodin integration guide
...
1. Require `schema_version == "1.0"` and the identity your
   scenario expects (`concept` = `bdx`, `phase` = `optimized`).
...
- Body frame (GLB and dynamics): X forward, +Y left, +Z up, origin at
  the CG, which sits at geometry [1.4101, 0, 0.125161] m.
- Transform: x_b = 1.4101 - x_g; y_b = -y_g;
  z_b = z_g - 0.125161 (matrix in `frames.geometry_to_body_matrix`).
...
References: S = 1.10757 m^2, b = 2.56237 m, c = 0.452397 m (MAC). Mass state:
20.7965 kg with 2.19654 kg fuel aboard (`mass_properties`). With alpha in
radians and tail_incidence held at -1.68945 deg:

```text
CL = -0.0595619 + 5.09325*alpha
Cm = 0.0354466 - 0.754003*alpha   (about the CG)
CD = 0.0349707 + 0.0467443*CL^2
```

Dimensionalize with qbar = 0.5*rho*V^2 and apply the section 2 adapter.
Initialize from `trim_map.csv` (cruise row: 310.018 m, 41.4827 m/s TAS, alpha
2.69354 deg, throttle 0.221823); re-solve equilibrium for any other condition
...
deg; Re/m 2.7714e+06 (single tabulated condition). Policy
